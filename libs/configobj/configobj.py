...
class ConfigObj(Section):
    ...
    def _load(self, infile, configspec):
        ...
        assert all(isinstance(line, six.string_types)
                   for line in content), repr(content)
        content = [line.rstrip('\r\n') for line in content]
    ...
    def _parse(self, infile):
        ...
        while cur_index < maxline:
            ...
            if mat is None:
                self._handle_error(
                    'Invalid line ({0!r}) (matched as neither section nor keyword) at line "%s".'.format(
                        line),
        ...
        for entry in configspec.sections:
            ...
            if entry not in section:
                ...
                if copy:
                    # copy comments
                    section.comments[
                        entry] = configspec.comments.get(entry, [])
                    section.inline_comments[
                        entry] = configspec.inline_comments.get(entry, '')
    ...
    def validate(self, validator, preserve_errors=False, copy=False,
        ...
        def validate_entry(entry, spec, val, missing, ret_true, ret_false):
            ...
            try:
                section.default_values[entry] = validator.get_default_value(configspec[
                                                                            entry])
