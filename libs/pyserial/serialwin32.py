...
class Win32Serial(SerialBase):
    ...
    def _reconfigurePort(self):
        ...
        win32.SetCommTimeouts(self.hComPort, ctypes.byref(
            win32.COMMTIMEOUTS(*timeouts)))
        ...
        if self._bytesize == FIVEBITS:
            ...
            raise ValueError(
                "Unsupported number of data bits: %r" % self._bytesize)
        ...
        if self._stopbits == STOPBITS_ONE:
            ...
            raise ValueError(
                "Unsupported number of stop bits: %r" % self._stopbits)
        ...
        if not win32.SetCommState(self.hComPort, ctypes.byref(comDCB)):
            raise ValueError(
                "Cannot configure port, some setting was wrong. Original message: %s" % ctypes.WinError())
    #~ def __del__(self):
        #~ self.close()
        ...
        if size > 0:
            ...
            if self.timeout == 0:
                ...
                if n > 0:
                    ...
                    rc = win32.DWORD()
                    err = win32.ReadFile(self.hComPort, buf, n, ctypes.byref(
                        rc), ctypes.byref(self._overlappedRead))
                    if not err and win32.GetLastError() != win32.ERROR_IO_PENDING:
                        raise SerialException(
                            "ReadFile failed (%s)" % ctypes.WinError())
                    err = win32.WaitForSingleObject(
                        self._overlappedRead.hEvent, win32.INFINITE)
                ...
                rc = win32.DWORD()
                err = win32.ReadFile(self.hComPort, buf, size, ctypes.byref(
                    rc), ctypes.byref(self._overlappedRead))
                if not err and win32.GetLastError() != win32.ERROR_IO_PENDING:
                    raise SerialException(
                        "ReadFile failed (%s)" % ctypes.WinError())
                err = win32.GetOverlappedResult(self.hComPort, ctypes.byref(
                    self._overlappedRead), ctypes.byref(rc), True)
                read = buf.raw[:rc.value]
        else:
            read = bytes()
        return bytes(read)
    ...
    def write(self, data):
        ...
        if data:
            ...
            n = win32.DWORD()
            err = win32.WriteFile(self.hComPort, data, len(
                data), ctypes.byref(n), self._overlappedWrite)
            if not err and win32.GetLastError() != win32.ERROR_IO_PENDING:
                raise SerialException(
                    "WriteFile failed (%s)" % ctypes.WinError())
            # if blocking (None) or w/ write timeout (>0)
            if self._writeTimeout != 0:
                # Wait for the write to complete.
                #~ win32.WaitForSingleObject(self._overlappedWrite.hEvent, win32.INFINITE)
                err = win32.GetOverlappedResult(
                    self.hComPort, self._overlappedWrite, ctypes.byref(n), True)
                if n.value != len(data):
    ...
    rtsToggle = property(getRtsToggle, setRtsToggle,
                         doc="RTS toggle control setting")
