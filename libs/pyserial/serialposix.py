...
if plat[:5] == 'linux':    # Linux (confirmed)
    ...
    def set_special_baudrate(port, baudrate):
        raise ValueError(
            "sorry don't know how to handle non standard baud rate on this platform")
    baudrate_constants = {}
...
class PosixSerial(SerialBase):
    ...
    def open(self):
        ...
        try:
            ...
            self.fd = None
            raise SerialException(
                "could not open port %s: %s" % (self._port, msg))
    ...
    def _reconfigurePort(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if custom_baud < 0:
                        raise ValueError('Invalid baud rate: %r' %
                                         self._baudrate)
        # setup char len
        cflag &= ~TERMIOS.CSIZE
        if self._bytesize == 8:
        ...
        if self._stopbits == STOPBITS_ONE:
            ...
            raise ValueError(
                'Invalid stop bit specification: %r' % self._stopbits)
        # setup parity
...
class PosixPollSerial(Serial):
    ...
    def read(self, size=1):
        ...
        read = bytearray()
        poll = select.poll()
        poll.register(self.fd, select.POLLIN | select.POLLERR |
